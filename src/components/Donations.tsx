...
const Donations: React.FC = () => {
  ...
  const causes = [
    {
      id: 'general',
      title: t('donations.causes.general.title'),
      description: t('donations.causes.general.description'),
      raised: 2450000,
      target: 5000000,
      donors: 12500,
      icon: Star,
    ...
    {
      id: 'annadanam',
      title: t('donations.causes.annadanam.title'),
      description: t('donations.causes.annadanam.description'),
      raised: 1850000,
      target: 3000000,
      donors: 8750,
      icon: Gift,
    ...
    {
      id: 'education',
      title: t('donations.causes.education.title'),
      description: t('donations.causes.education.description'),
      raised: 950000,
      target: 2000000,
      donors: 4200,
      icon: Target,
  ...
  const divineMessages = [
    t('donations.divine_messages.0'),
    t('donations.divine_messages.1'),
    t('donations.divine_messages.2'),
    t('donations.divine_messages.3'),
    t('donations.divine_messages.4'),
    t('donations.divine_messages.5'),
    t('donations.divine_messages.6'),
    t('donations.divine_messages.7')
  ];
  ...
  const handleDonate = () => {
    if (!donorName || !donorPhone || !donorEmail) {
      alert(t('donations.validation.fill_all_fields'));
      return;
    }

    if (donationAmount <= 0) {
      alert(t('donations.validation.valid_amount'));
      return;
    }
  ...
  const handleDownloadReceipt = () => {
    // Simulate receipt download
    const receiptContent = `
      ${t('donations.receipt.header')}
      --------------------------
      ${t('donations.receipt.receipt_no')}: ${donationData.receiptNo}
      ${t('donations.receipt.date')}: ${donationData.date}
      ${t('donations.receipt.time')}: ${donationData.time}
      
      ${t('donations.receipt.donor_info')}:
      ${t('donations.receipt.name')}: ${donationData.donorName}
      ${t('donations.receipt.phone')}: ${donationData.donorPhone}
      ${t('donations.receipt.email')}: ${donationData.donorEmail}
      
      ${t('donations.receipt.donation_details')}:
      ${t('donations.receipt.amount')}: ₹${donationData.amount}
      ${t('donations.receipt.cause')}: ${donationData.cause}
      
      ${donationData.divineMessage}
      
      ${t('donations.receipt.thank_you')}
      ${t('donations.receipt.computer_generated')}
      ${t('donations.receipt.tax_exemption')}
    `;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl w-full max-w-4xl">
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-2xl font-bold text-gray-900">{t('donations.receipt.success_title')}</h2>
            <button
              onClick={handleNewDonation}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
            ...
            <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl p-6 border-l-4 border-orange-500">
              <div className="flex items-start space-x-3 h-full">
                <CheckCircle className="w-6 h-6 text-green-600 mt-1 flex-shrink-0" />
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3 text-lg">{t('donations.receipt.divine_blessings')}</h3>
                  <p className="text-gray-700 italic text-base leading-relaxed">"{donationData.divineMessage}"</p>
                  
                  {/* Additional spiritual quote */}
                  <div className="mt-4 pt-4 border-t border-orange-200">
                    <p className="text-sm text-orange-700">
                      {t('donations.receipt.additional_quote')}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="text-center mb-4">
                ...
                <h3 className="text-lg font-bold text-gray-900">
                  {t('donations.receipt.thank_you_name', { name: donationData.donorName })}
                </h3>
                <p className="text-gray-600">{t('donations.receipt.received_successfully')}</p>
              </div>

              <div className="bg-gray-50 rounded-xl p-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">{t('donations.receipt.receipt_no')}:</span>
                    <div className="font-semibold text-gray-900">{donationData.receiptNo}</div>
                  </div>
                  <div>
                    <span className="text-gray-600">{t('donations.receipt.date_time')}:</span>
                    <div className="font-semibold text-gray-900">{donationData.date}<br/>{donationData.time}</div>
                  </div>
                  <div className="col-span-2">
                    <span className="text-gray-600">{t('donations.receipt.cause')}:</span>
                    <div className="font-semibold text-gray-900">{donationData.cause}</div>
                  </div>
                  <div className="col-span-2">
                    <span className="text-gray-600">{t('donations.receipt.amount')}:</span>
                    <div className="font-bold text-green-600 text-xl">₹{donationData.amount}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                  <Download className="w-4 h-4" />
                  {t('donations.receipt.download_receipt')}
                </button>
                <button
                  onClick={handleNewDonation}
                  className="w-full bg-gray-200 text-gray-700 py-3 rounded-xl font-semibold hover:bg-gray-300 transition-colors"
                >
                  {t('donations.receipt.another_donation')}
                </button>
              </div>
              ...
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-xs text-blue-700">
                  {t('donations.receipt.tax_benefit_info')}
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    <section id="donations" className="py-20 bg-gradient-to-b from-orange-50 to-yellow-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">
            {t('donations.section_title')}
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            {t('donations.section_subtitle')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Donation Form */}
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">{t('donations.form.title')}</h3>
            
            {/* Cause Selection */}
            <div className="mb-6">
              <label className="block text-sm font-semibold text-gray-700 mb-3">
                {t('donations.form.choose_cause')}
              </label>
              <div className="space-y-3">
                {causes.map((cause) => {
                  const IconComponent = cause.icon;
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                <input
                  type="number"
                  placeholder={t('donations.form.custom_amount_placeholder')}
                  value={customAmount}
                  onChange={(e) => {
                    setCustomAmount(e.target.value);
                    setSelectedAmount(0);
            ...
            <div className="mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input
                  type="tel"
                  placeholder={t('donations.form.phone_placeholder')}
                  value={donorPhone}
                  onChange={(e) => setDonorPhone(e.target.value)}
                  className="py-3 px-4 border-2 border-gray-200 rounded-xl focus:border-orange-500 focus:outline-none"
                  required
            ...
            <button 
              ...
              className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white py-4 rounded-xl font-bold text-lg hover:from-orange-600 hover:to-red-600 transition-all flex items-center justify-center"
            >
              {t('donations.form.donate_button', { amount: donationAmount })}
              <ArrowRight className="w-5 h-5 ml-2" />
            </button>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-xl p-8">
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold text-gray-700">{t('donations.progress.progress')}</span>
                  <span className="text-sm font-semibold text-gray-700">
                    {progressPercentage.toFixed(1)}%
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  ...
                  <div className="text-sm text-gray-600">{t('donations.progress.donors')}</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-gray-900">
                    ₹{(currentCause.target - currentCause.raised).toLocaleString()}
                  </div>
                  <div className="text-sm text-gray-600">{t('donations.progress.to_go')}</div>
                </div>
              </div>
            </div>
            ...
            {/* Recent Donations */}
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h4 className="text-lg font-bold text-gray-900 mb-4">{t('donations.recent_contributions.title')}</h4>
              <div className="space-y-4">
                {[
                  { name: t('donations.recent_contributions.donors.0.name'), amount: 5000, time: t('donations.recent_contributions.donors.0.time') },
                  { name: t('donations.recent_contributions.donors.1.name'), amount: 1000, time: t('donations.recent_contributions.donors.1.time') },
                  { name: t('donations.recent_contributions.donors.2.name'), amount: 2500, time: t('donations.recent_contributions.donors.2.time') },
                  { name: t('donations.recent_contributions.donors.3.name'), amount: 500, time: t('donations.recent_contributions.donors.3.time') },
                ].map((donation, index) => (
                  <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-gradient-to-r from-orange-400 to-red-400 rounded-full flex items-center justify-center">
