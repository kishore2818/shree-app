...
//       image: new URL('../assets/s.jpg', import.meta.url).href,
//       rating: 4.9,
//       currentQueue: 2500,
...
//       // image: 'ff.jpg',
//       image: new URL('../assets/ff.jpg', import.meta.url).href,
//       rating: 4.8,
//       currentQueue: 850,
...
//             image: new URL('../assets/Ambaji-Temple-Banner.jpg', import.meta.url).href,
...
const TempleInfo: React.FC = () => {
  ...
  const temples = [
    {
      id: '1',
      name: t('templeInfo.temples.0.name'),
      deity: t('templeInfo.temples.0.deity'),
      location: t('templeInfo.temples.0.location'),
      timings: t('templeInfo.temples.0.timings'),
      description: t('templeInfo.temples.0.description'),
      image: new URL('../assets/s.jpg', import.meta.url).href,
      rating: 4.9,
      currentQueue: 2500,
      estimatedWait: t('templeInfo.temples.0.estimatedWait'),
      specialEvents: t('templeInfo.temples.0.specialEvents'),
      history: t('templeInfo.temples.0.history'),
      mapUrl: 'https://maps.googleapis.com/maps/api/staticmap?center=Somnath+Temple,Veraval,Gujarat&zoom=15&size=600x400&maptype=roadmap&markers=color:red%7Clabel:T%7C20.8880,70.4015&key=YOUR_API_KEY'
    },
    {
      id: '2',
      name: t('templeInfo.temples.1.name'),
      deity: t('templeInfo.temples.1.deity'),
      location: t('templeInfo.temples.1.location'),
      timings: t('templeInfo.temples.1.timings'),
      description: t('templeInfo.temples.1.description'),
      image: new URL('../assets/ff.jpg', import.meta.url).href,
      rating: 4.8,
      currentQueue: 850,
      estimatedWait: t('templeInfo.temples.1.estimatedWait'),
      specialEvents: t('templeInfo.temples.1.specialEvents'),
      history: t('templeInfo.temples.1.history'),
      mapUrl: 'https://maps.googleapis.com/maps/api/staticmap?center=Dwarkadhish+Temple,Dwarka,Gujarat&zoom=15&size=600x400&maptype=roadmap&markers=color:red%7Clabel:T%7C22.2405,68.9686&key=YOUR_API_KEY'
    },
    {
      id: '3',
      name: t('templeInfo.temples.2.name'),
      deity: t('templeInfo.temples.2.deity'),
      location: t('templeInfo.temples.2.location'),
      timings: t('templeInfo.temples.2.timings'),
      description: t('templeInfo.temples.2.description'),
      image: new URL('../assets/Ambaji-Temple-Banner.jpg', import.meta.url).href,
      rating: 4.9,
      currentQueue: 1200,
      estimatedWait: t('templeInfo.temples.2.estimatedWait'),
      specialEvents: t('templeInfo.temples.2.specialEvents'),
      history: t('templeInfo.temples.2.history'),
      mapUrl: 'https://maps.googleapis.com/maps/api/staticmap?center=Ambaji+Temple,Banas+Kantha,Gujarat&zoom=15&size=600x400&maptype=roadmap&markers=color:red%7Clabel:T%7C24.3316,72.8513&key=YOUR_API_KEY'
    }
  ];
  ...
  return (
    <section id="temples" className="py-20 bg-orange-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">
            {t('templeInfo.section_title')}
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            {t('templeInfo.section_subtitle')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          {/* Temple Selection */}
          <div className="lg:col-span-1">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('templeInfo.featured_temples')}</h3>
            <div className="space-y-4">
              {temples.map((temple, index) => (
                <button
                  key={temple.id}
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3">
                      <Star className="w-5 h-5 text-orange-600" />
                      <span className="text-gray-700">{currentTemple.rating} {t('templeInfo.rating')}</span>
                    </div>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="bg-white rounded-lg p-4 shadow-sm">
                      <h4 className="font-semibold text-gray-900 mb-2">{t('templeInfo.live_queue_status')}</h4>
                      <div className="flex justify-between items-center">
                        <span className="text-2xl font-bold text-red-600">{currentTemple.currentQueue}</span>
                        <span className="text-sm text-gray-600">{t('templeInfo.people_in_queue')}</span>
                      </div>
                      <p className="text-sm text-orange-600 mt-1">{t('templeInfo.est_wait')}: {currentTemple.estimatedWait}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 mb-6">
                  <div className="flex items-center">
                    <Calendar className="w-5 h-5 text-yellow-600 mr-2" />
                    <span className="font-semibold text-yellow-800">{t('templeInfo.upcoming_event')}:</span>
                  </div>
                  <p className="text-yellow-700 mt-1">{currentTemple.specialEvents}</p>
                </div>
                ...
                <div className="flex flex-wrap gap-4">
                  <button className="bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">
                    {t('templeInfo.buttons.book_darshan')}
                  </button>
                  <button 
                    onClick={handleGetDirections}
                    className="bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors flex items-center"
                  >
                    <Navigation className="w-5 h-5 mr-2" />
                    {t('templeInfo.buttons.get_directions')}
                  </button>
                  <button 
                    onClick={handleVirtualTour}
                    className="bg-green-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-700 transition-colors"
                  >
                    {t('templeInfo.buttons.history')}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-4xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-gray-200 p-6 rounded-t-2xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-2xl font-bold text-gray-900">
                    {currentTemple.name} - {t('templeInfo.history_modal.title')}
                  </h3>
                  <button 
                    onClick={() => setShowHistory(false)}
                    className="text-gray-500 hover:text-gray-700 transition-colors"
              ...
              <div className="p-6">
                ...
                <div className="mt-8 flex justify-end">
                  <button 
                    ...
                    className="bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors"
                  >
                    {t('templeInfo.history_modal.close_button')}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-4xl w-full">
              <div className="sticky top-0 bg-white border-b border-gray-200 p-6 rounded-t-2xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-2xl font-bold text-gray-900">
                    {t('templeInfo.map_modal.title', { templeName: currentTemple.name })}
                  </h3>
                  <button 
                    onClick={() => setShowMap(false)}
                    className="text-gray-500 hover:text-gray-700 transition-colors"
              ...
              <div className="p-6">
                ...
                <div className="bg-gray-100 rounded-lg overflow-hidden mb-6">
                  <div className="aspect-video bg-gradient-to-br from-blue-50 to-green-50 flex items-center justify-center relative">
                    ...
                    <div className="text-center">
                      ...
                      <h4 className="text-xl font-bold text-gray-900 mb-2">{currentTemple.name}</h4>
                      <p className="text-gray-600">{currentTemple.location}</p>
                      <p className="text-sm text-gray-500 mt-4">{t('templeInfo.map_modal.map_placeholder')}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="bg-blue-50 rounded-lg p-4">
                    <h4 className="font-semibold text-blue-900 mb-2">{t('templeInfo.map_modal.address_title')}</h4>
                    <p className="text-blue-800">{currentTemple.name}</p>
                    <p className="text-blue-700">{currentTemple.location}</p>
                    <p className="text-blue-600 text-sm mt-2">{t('templeInfo.map_modal.address_state')}</p>
                  </div>
                  <div className="bg-green-50 rounded-lg p-4">
                    <h4 className="font-semibold text-green-900 mb-2">{t('templeInfo.map_modal.travel_time')}</h4>
                    <p className="text-green-800 text-2xl font-bold">{t('templeInfo.map_modal.travel_duration')}</p>
                    <p className="text-green-700 text-sm">{t('templeInfo.map_modal.travel_from_city')}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4 justify-between">
                  <div className="flex flex-wrap gap-4">
                    <button className="bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors flex items-center">
                      <Navigation className="w-5 h-5 mr-2" />
                      {t('templeInfo.map_modal.open_google_maps')}
                    </button>
                    <button className="bg-gray-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-gray-700 transition-colors">
                      {t('templeInfo.map_modal.download_directions')}
                    </button>
                  </div>
                  <button 
                    onClick={() => setShowMap(false)}
                    className="bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors"
                  >
                    {t('templeInfo.map_modal.close_map')}
                  </button>
                </div>

                {/* Quick Tips */}
                <div className="mt-6 bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
                  <h4 className="font-semibold text-yellow-800 mb-2">{t('templeInfo.map_modal.travel_tips')}</h4>
                  <ul className="text-yellow-700 text-sm space-y-1">
                    <li>{t('templeInfo.map_modal.tips.0')}</li>
                    <li>{t('templeInfo.map_modal.tips.1')}</li>
                    <li>{t('templeInfo.map_modal.tips.2')}</li>
                    <li>{t('templeInfo.map_modal.tips.3')}</li>
                  </ul>
                </div>
              </div>
            </div>
