...
const VirtualDarshan: React.FC = () => {
  ...
  const liveStreams = [
    {
      id: '1',
      temple: t('virtualDarshan.streams.0.temple'),
      location: t('virtualDarshan.streams.0.location'),
      viewers: 15420,
      quality: t('virtualDarshan.streams.0.quality'),
      language: t('virtualDarshan.streams.0.language'),
      nextAarti: t('virtualDarshan.streams.0.nextAarti'),
      thumbnail: new URL('../assets/s.jpg', import.meta.url).href,
      youtubeId: 'MVqqIh15OMs'
    },
    {
      id: '2',
      temple: t('virtualDarshan.streams.1.temple'),
      location: t('virtualDarshan.streams.1.location'),
      viewers: 8650,
      quality: t('virtualDarshan.streams.1.quality'),
      language: t('virtualDarshan.streams.1.language'),
      nextAarti: t('virtualDarshan.streams.1.nextAarti'),
      thumbnail: new URL('../assets/ff.jpg', import.meta.url).href,
      youtubeId: 'MVqqIh15OMs'
    },
    {
      id: '3',
      temple: t('virtualDarshan.streams.2.temple'),
      location: t('virtualDarshan.streams.2.location'),
      viewers: 12340,
      quality: t('virtualDarshan.streams.2.quality'),
      language: t('virtualDarshan.streams.2.language'),
      nextAarti: t('virtualDarshan.streams.2.nextAarti'),
      thumbnail: new URL('../assets/Ambaji-Temple-Banner.jpg', import.meta.url).href,
      youtubeId: 'MVqqIh15OMs'
    }
  ];
  ...
  return (
    <section id="virtual" className="py-20 bg-gradient-to-b from-indigo-50 to-purple-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-900 mb-4">
            {t('virtualDarshan.section_title')}
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            {t('virtualDarshan.section_subtitle')}
          </p>
        </div>
        ...
        <div className="mb-12">
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="relative">
              ...
              <div className="aspect-video bg-black">
                {isPlaying ? (
                  <iframe
                    ...
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    title={t('virtualDarshan.live_darshan')}
                  />
                ) : (
                  <div className="w-full h-full bg-gradient-to-r from-yellow-200 to-yellow-200 flex items-center justify-center relative">
                    <img 
                      src={liveStreams[0].thumbnail} 
                      alt={t('virtualDarshan.thumbnail_alt')}
                      className="w-full h-full object-cover absolute inset-0"
                    />
                    <div className="absolute inset-0 bg-black bg-opacity-40"></div>
                    <button 
                    ...
                    >
                      ...
                      <h3 className="text-2xl font-bold mb-2">{t('virtualDarshan.live_darshan')}</h3>
                      <p className="text-purple-200">{t('virtualDarshan.click_to_play')}</p>
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="absolute top-4 left-4 bg-red-600 text-white px-3 py-1 rounded-full flex items-center space-x-2">
                <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
                <span className="text-sm font-semibold">{t('virtualDarshan.live_badge')}</span>
              </div>
            ...
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h4 className="text-xl font-bold text-gray-900">{t('virtualDarshan.current_stream.title')}</h4>
                  <p className="text-gray-600">{t('virtualDarshan.current_stream.description')}</p>
                </div>
                <div className="flex items-center space-x-4 text-sm text-gray-500">
                  <span className="flex items-center">
                    <Video className="w-4 h-4 mr-1" />
                    {t('virtualDarshan.current_stream.quality')}
                  </span>
                  <span className="flex items-center">
                    <Clock className="w-4 h-4 mr-1" />
                    {t('virtualDarshan.current_stream.next_aarti')}
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Other Live Streams */}
        <div>
          <h3 className="text-2xl font-bold text-gray-900 mb-8">{t('virtualDarshan.more_streams_title')}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {liveStreams.map((stream) => (
              <div key={stream.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <div className="relative">
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-2 text-xs text-gray-500">
                    <div className="flex justify-between">
                      <span>{t('virtualDarshan.stream_info.quality')}:</span>
                      <span className="font-medium">{stream.quality}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t('virtualDarshan.stream_info.languages')}:</span>
                      <span className="font-medium">{stream.language}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t('virtualDarshan.stream_info.next_aarti')}:</span>
                      <span className="font-medium text-orange-600">{stream.nextAarti}</span>
                    </div>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full mt-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-2 rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-colors"
                  >
                    {t('virtualDarshan.watch_live_button')}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="mt-16 bg-white rounded-2xl shadow-lg p-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h4 className="font-bold text-lg text-gray-900 mb-2">{t('virtualDarshan.features.0.title')}</h4>
              <p className="text-gray-600">{t('virtualDarshan.features.0.description')}</p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Users className="w-8 h-8 text-green-600" />
              </div>
              <h4 className="font-bold text-lg text-gray-900 mb-2">{t('virtualDarshan.features.1.title')}</h4>
              <p className="text-gray-600">{t('virtualDarshan.features.1.description')}</p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Clock className="w-8 h-8 text-orange-600" />
              </div>
              <h4 className="font-bold text-lg text-gray-900 mb-2">{t('virtualDarshan.features.2.title')}</h4>
              <p className="text-gray-600">{t('virtualDarshan.features.2.description')}</p>
            </div>
          </div>
        </div>
      </div>
